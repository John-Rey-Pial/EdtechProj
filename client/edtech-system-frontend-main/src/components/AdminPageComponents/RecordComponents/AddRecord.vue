	...
	<div class="addRecord">
		<form action="" @submit.prevent="addRecord">
			...
			<div class="btnAction">
				...
				<v-btn color="yellow darken-2" type="submit">Submit Record</v-btn>
				<v-dialog hide-overlay persistent width="300" v-model="dialog2">
					<v-card color="white" light>
						<v-card-text>
							<p mt-5>Adding Record. Please wait</p>
							<v-progress-linear
								indeterminate
								color="black"
								class="mb-0 mt-5"
		...
		methods: {
			...
			async addRecord() {
				if (this.record.borrowed_equipments.length == 0) {
					...
					try {
						this.dialog2 = true;
						await recordAPI.prototype.createRecord(this.record);

						alert("Success!");
						this.dialog2 = false;
					} catch (error) {
						alert("Incorrect Release Code");
						this.dialog2 = false;
					}
				}
			},
